% In this session human face pictures will be shown to the participants.
...
% variables will be saved into session1.mat file and FA rate , Hit rate (in
% 2 forms: raw(with name raw) and percentage(with name all) in external
% function analyze_data will be saved into session1_analysis.mat file.
function session1
    outfile = 'session1.mat';
    size1 = 100;
    size2 = 50;
    % for examle img_tracker(100, :)  => 1 4
    % means that img images/humans/100.jpg was perceived by the participant as
    ...
    all_images = session1_part1(size1, size2);
    ...
    [user_new, user_old] = session1_part2(all_images2);
    res = session1_part3(old_images, user_new, user_old);
    disp(res);
    title_str = 'ROC Curve for human faces';
    analyze_data(res, title_str, 'session1_analysis.mat');
    
    function res = session1_part1(size1, size2)
        res = randperm(size1 , size1)';
        
        for n=1:size2
            img_name = strcat('images/humans/',int2str(res(n,1)),'.jpg');
            img = imread(img_name);
    ...
    function [user_new, user_old] = session1_part2(all_images)
        ...
        for n=1:size1
            img_name = strcat('images/humans/',int2str(all_images(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
            waitforbuttonpress;
    ...
    function [res] = session1_part3(old_images , user_new, user_old)
        res = zeros(10, 2);
        ...
        for n = 1:size(user_new)
            img_name = strcat('images/humans/',int2str(user_new(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
        ...
        for n = 1:size(user_old)
            img_name = strcat('images/humans/',int2str(user_old(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
