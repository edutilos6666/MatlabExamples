% In this session spider pictures will be shown to the participants.
...
% variables will be saved into session2.mat file and FA rate , Hit rate (in
% 2 forms: raw(with name raw) and percentage(with name all) in external
% function analyze_data will be saved into session2_analysis.mat file.
function session2
    outfile = 'session2.mat';
    size1 = 100;
    size2 = 50;
    % for examle img_tracker(100, :)  => 1 4
    % means that img images/spiders/100.jpg was perceived by the participant as
    ...
    all_images = session2_part1(size1, size2);
    ...
    [user_new, user_old] = session2_part2(all_images2);
    res = session2_part3(old_images, user_new, user_old);
    disp(res);
    title_str = 'ROC Curve for spider pictures';
    analyze_data(res, title_str, 'session2_analysis.mat');
    
    function res = session2_part1(size1, size2)
        res = randperm(size1 , size1)';
        
        for n=1:size2
            img_name = strcat('images/spiders/',int2str(res(n,1)),'.jpg');
            img = imread(img_name);
    ...
    function [user_new, user_old] = session2_part2(all_images)
        ...
        for n=1:size1
            img_name = strcat('images/spiders/',int2str(all_images(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
            waitforbuttonpress;
    ...
    function [res] = session2_part3(old_images , user_new, user_old)
        res = zeros(10, 2);
        ...
        for n = 1:size(user_new)
            img_name = strcat('images/spiders/',int2str(user_new(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
        ...
        for n = 1:size(user_old)
            img_name = strcat('images/spiders/',int2str(user_old(n,1)),'.jpg');
            img = imread(img_name);
            img_gray = rgb2gray(img);
            imshow(img_gray);
